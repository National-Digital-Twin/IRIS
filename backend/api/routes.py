...
async def get_epc_ratings_overtime(
    db: Annotated[AsyncSession, Depends(get_db)],
    polygon: Annotated[Optional[GeoJSONPolygon], Query()] = None,
    area_level: Annotated[Optional[str], Query()] = None,
    area_names: Annotated[Optional[List[str]], Query()] = None,
):
...
def _merge_ngd_pg_fallback(
    pg: DetailedBuildingSchema,
...
async def _apply_ngd_pg_fallback_if_required(
    uprn: str,
    db: AsyncSession,
    ngd_roof_material_results,
    ngd_solar_panel_presence_results,
...
async def _apply_epc_pg_fallback_if_required(
    building: DetailedBuilding, db: AsyncSession, uprn: str
):
...
async def get_building_by_uprn(
    ...
    (
        ...
        ngd_roof_shape_results,
        ngd_roof_aspect_areas_results,
    ) = await _apply_ngd_pg_fallback_if_required(
        uprn,
        db,
        ngd_roof_material_results,
        ngd_solar_panel_presence_results,
    ...
    await _apply_epc_pg_fallback_if_required(building, db, uprn)
